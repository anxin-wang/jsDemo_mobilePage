...
function getArticleTopicId() {
	var hash = location.hash;
	var topic_id = hash.slice(hash.indexOf("topic_id"),hash.indexOf("source")-1).slice(8);
	
	return topic_id;
}
function getArticleTopicName() {
	var hash = location.hash;
	var topic_name = hash.slice(hash.indexOf("topic_name"),hash.indexOf("topic_id")-1).slice(10);
...
$("#entire-article").live("pageshow", function() {
    ...
    //话题id,@ id
    var topic_ids=null,parent_topic_ids=null,parent_at_ids=null;
    if(window.localStorage.getItem("topic_ids")!=null && window.localStorage.getItem("topic_ids").length>0){                      
        topic_ids=window.localStorage.getItem("topic_ids").split(",");
    }
    if(window.localStorage.getItem("article_parenttweet_topic_ids")!=null && window.localStorage.getItem("article_parenttweet_topic_ids").length>0){    
        parent_topic_ids=window.localStorage.getItem("article_parenttweet_topic_ids").split(","); 
    }
    var at_ids=null,parent_at_ids=null,parent_at_ids=null;
  ...
  $(".article-body-words a").each(function(index,item){
  	if($(item).text().indexOf("@")>=0){
        ...
        //if(topic_ids!=null && topic_ids.length>0){
             $(item).attr("topic_id",topic_ids[index])                     
        //}
  ...
  $(".weibo-article-source a").each(function(index,item){
      ...
      $(item).attr("topic_id",parent_topic_ids[index])                          
    } 
  })
