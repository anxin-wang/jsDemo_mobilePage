	...
	function createWeiboItem(obj, container, opts) {
		...
		container.find(".weibo-list-item").last().click(function() {
			...
			if(obj.content) {
				...
				window.localStorage.setItem("article_content", obj.content);
			}
			if(obj.thumb) {
			...
			if(obj.parent_tweet) {
				...
				window.localStorage.setItem("article_parenttweet_content", obj.parent_tweet.content);
				if(obj.parent_tweet.thumb) {
            ...
            //token传给article.html  
			$.mobile.changePage("./article.html?token=" + opts.uid);
